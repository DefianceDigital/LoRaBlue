...
void loop()
{
  ...
  if (receivedFlag == true) {
    ...
    Serial.println();*/
    if (state == RADIOLIB_ERR_NONE) { // if CRC enabled, message will not be decrypted or bounced unless we have said message exactly right (we actually don't process it at all)
      if(DEFICHAT){
        if(incoming[0] == 0x00){
    ...
    } else if(state != RADIOLIB_ERR_CRC_MISMATCH) { // This means there is an error on recieved message but we ignore crc errors and do not report them
